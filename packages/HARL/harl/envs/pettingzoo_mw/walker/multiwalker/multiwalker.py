...
from .multiwalker_stable import MultiWalkerEnv as _env_stable
from .mw_obs_fric import MultiWalkerEnv as _env_fric
...
class raw_env(AECEnv[str, ObsType, ActionType], EzPickle):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.scenario == "default":
            ...
            if self.disturbs["friction"]["obs_enabled"]:
                ...
                if self.use_angle_reward:
                    self.env = _env_stable(*args, **kwargs)
                else:
                    self.env = _env_custom(*args, **kwargs)

        self.turbances_array = []

        self.is_eval = custom_parameters.get("is_eval", False)
    ...
    def set_disturbances(self, turbances_array):
        self.turbances_array = turbances_array
                ...
                if self.disturbs["friction"]["disturb_enabled"]:
                    if self.disturbance["is_disturbancing"]:
                        ...
                        if self.np_random.random() < 0.0025:
                            ...
                            for ter in self.env.terrain:
                                if self.disturbs["friction"].get("disturb_args", None):
                                    ter.fixtures[0].friction = self.disturbs[
                                        "friction"
                                    ]["disturb_args"]["friction"]
                                else:
