...
from mapdn.environments.var_voltage_control.disturbances.DisturbanceFactory import (
    DisturbanceFactory,
)
...
class VoltageControl(MultiAgentEnv):
    ...
    def get_obs(self) -> list[np.ndarray]:
        ...
        for i in range(len(self.powergrid.sgen)):
            ...
            clusters[f"sgen{i}"][0].loc[clusters[f"sgen{i}"][4]]["q_mvar"] += clusters[
                f"sgen{i}"
            ][3]
            for j in range(len(self.powergrid.sgen)):
                if i != j and clusters[f"sgen{j}"][1] == clusters[f"sgen{i}"][1]:
                    clusters[f"sgen{j}"][0].loc[clusters[f"sgen{i}"][4]]["p_mw"] += (
                        clusters[f"sgen{i}"][2]
                    )
    ...
    def _select_start_day(self):
        ...
        episode_days = (
            self.episode_limit // (24 * (60 // self.time_delta))
        ) + 1  # margin
    ...
    def _get_episode_pv_history(self):
        ...
        start = (
            self._episode_start_interval
            + self._episode_start_hour * (60 // self.time_delta)
            + self._episode_start_day * 24 * (60 // self.time_delta)
        )
