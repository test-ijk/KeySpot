...
def plot_df(df, color, xaxis, yaxis, ma=1, label=""):
    df = df[df[xaxis] <= 20000]
    df[yaxis] = pd.to_numeric(
        df[yaxis], errors="coerce"
    )  # convert NaN string to NaN value
    ...
    plt.fill_between(
        x, mean + std, mean - std, alpha=0.25, color=color, rasterized=True
    )
...
if __name__ == "__main__":
    ...
    prs.add_argument(
        "-yaxis",
        type=str,
        default="system_total_waiting_time",
    ...
    labels = (
        ...
        else cycle([str(i) for i in range(len(args.f))])
    )
